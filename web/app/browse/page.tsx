...
export default function BrowsePage() {
  ...
  useEffect(() => {
    const loadFolderContents = async () => {
      ...
      const foundFolders = data.filter(item => item.name && item.metadata === null).map(f => f.name);
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">📁 Browse: <span className="text-orange-400">{currentPath}</span></h1>
